...
class DummyHMD : public vr::ITrackedDeviceServerDriver, public vr::IVRDisplayComponent
{
	/// <summary>
	/// Makes a new instance
	/// </summary>
	/// <returns>A new DummyHMD</returns>
	static std::shared_ptr<DummyHMD> make_new();
	virtual ~DummyHMD() = default;

	/// <summary>
	/// Disable move and copy
	/// Because we give the pointer to this to VRServerDriverHost, we dont want it to ever change
	/// </summary>
	DummyHMD(DummyHMD&&) = delete;
	DummyHMD& operator=(DummyHMD&&) = delete;
	DummyHMD(const DummyHMD&) = delete;
	DummyHMD& operator= (const DummyHMD&) = delete;
